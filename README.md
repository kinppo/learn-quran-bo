...
Group forms retain teacher/auditor assignments, teaching language, meeting metadata and weekly sessions. Edits preserve planning IDs and validate overlaps. Teacher photos use private MinIO uploads assigned by Admin to the teacher. Student details include account data and API statistics. The home dashboard shows live student, teacher, group and program totals, the five oldest waiting student accounts, recent registrations and management shortcuts. Review links preserve the waiting-status filter; each panel handles loading, empty and error states independently. Payment controls and unsupported dead navigation links are absent; no replacement features are invented.
